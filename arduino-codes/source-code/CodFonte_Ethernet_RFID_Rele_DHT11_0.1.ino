...
void Perifericos(){
  if(EmergenciaValue == 1){
    int l = 0;
    while(EmergenciaValue == 1){
      
      l = l + 1;
      Serial.println(l);
      fill_solid(leds_2, LED_COUNT, CRGB(250, 0, 0));
      fill_solid(leds_1, LED_COUNT, CRGB(0, 0, 255));
      FastLED.show();
      tone(buzzer_1, 999);
      //tone(buzzer_2, 75);
      delay(120);
  ...
  if(P001Value == 2){
    ...
    //COLOCAR COD HEX DEC. PARA LIGAR 
    irsend.sendNEC(0x20DF10EF, 32); // Envie o código NEC com 32 bits
  }

  if(P001Value == 3){
    digitalWrite(releLEDQuadro, HIGH);
    //COLOCAR COD HEX DEC. PARA DESLIGAR
  ...
  if(P001Value == 4){
    //COLOCAR COD HEX DEC. PARA CONGELAR
...
void ChekValuesServer(){
  ...
  while (client.connected()) {
    if (client.available()) {
      ...
      EmergenciaValue = getValue(response, "EMERG:");
      ...
      if (onOffStart >= 0) {
        ...
        if (onOffEnd >= 0) {
          onOffCode = response.substring(onOffStart + 6, onOffEnd);
        }
      }
      ...
      if (freezeStart >= 0) {
        ...
        if (freezeEnd >= 0) {
          freezeCode = response.substring(freezeStart + 7, freezeEnd);
        }
      }
