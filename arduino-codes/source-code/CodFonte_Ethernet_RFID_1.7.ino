...
void LeituraPerifericos(){
  ...
  if (mfrc522_3.PICC_IsNewCardPresent() && mfrc522_3.PICC_ReadCardSerial()) {
    ...
    if (status == true) {
      Access(true, "periféricos (professor)", "leds_3", "buzzer_3");
    } else{
      Access(false, "periféricos (professor)", "leds_3", "buzzer_3");
    }
...
void Access(bool access, String modulo, String led, String buzzer){
  if(access == true){
    Serial.println("Tag detectada no banco de dados: " + IDtag);
    Serial.println("Modulo receptor: " + modulo);
    Serial.println("");

    for(int i = 0; i < 2; i++){
      fill_solid(led, LED_COUNT, CRGB(0, 250, 0));
      FastLED.show();
      tone(buzzer, 1000);
      delay(100);
      noTone(buzzer);
  ...
  if(access == false){
    ...
    for(int i = 0; i < 3; i++){
      fill_solid(led, LED_COUNT, CRGB(250, 0, 0));
      FastLED.show();
      tone(buzzer, 100);
      delay(100);
      fill_solid(led, LED_COUNT, CRGB(0, 0, 0));
      FastLED.show();
      noTone(buzzer);
      delay(100);
    }
