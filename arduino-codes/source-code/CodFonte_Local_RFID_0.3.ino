...
void setup() {
  ...
  LedPiscar(strip.Color(255, 150, 0), 4, 500, 0);
  delay(500); 
...
void LeituraTags(){
  ...
  for (int i = 0; i < sizeof(storedTags) / sizeof(storedTags[0]); i++) {
    if (IDtag.startsWith(storedTags[i])) {
      ...
      LedPiscar(strip.Color(0, 255, 0), 2, 200, 0);
      found = true;
      break;
    }
  }
  ...
  if (!found) {
    ...
    LedPiscar(strip.Color(255, 0, 0), 3, 125, 1);
  }
...
void LedPiscar(uint32_t color, int vezes, int dell, int buzz) {
